...
uses
  ...
  Dialogs, StdCtrls, Grids, DB, DBClient, ExtCtrls;

const
  ActionStr: array[TReconcileAction] of string = ('Saltar', 'Abortar', 'Combinar',
    'Corregir', 'Cancelar', 'Refrescar');
  UpdateKindStr: array[TUpdateKind] of string = ('Modificación', 'Inserción',
    'Eliminación');
  UpdateKindStr2: array[TUpdateKind] of string = ('modificado', 'insertado',
    'eliminado');
    
  SCaption = 'Error al actualizar - %s';
  SUnchanged = '<sin cambios>';
  SBinary = '(Binario)';
  SFieldName = 'Nombre del Campo';
  SOriginal = 'Valor original';
  SConflict = 'Valor en conflicto';
  SValue = ' Valor';
  SNoData = '<Sin registros>';
  SNew = 'Nuevo';
...
resourcestring
   FKERROR = 'Existen restricciones a la hora de cambiar o eliminar esta fila:';
   FKDUP   = 'Ya existe una fila con esta clave: ';
   FKNULO  = 'Hay campos vacíos de deben de contener un valor.';
   FKLOCK  = 'Hay valores que fueron modificados por otro usuario.';
...
begin
  ...
  begin
    ...
    Inc(FColCount);
    UpdateData.Cells[FNewColIdx,0] := SValue +  ' ' + UpdateKindStr2[FUpdateKind];
  end else
  begin
...
begin
  ...
  // Intento de hacer más legible el error
  cBuf := lowercase(FError.Message);

  if pos('foreign key',cBuf)   > 0 then  lMsgErr.Caption := FKERROR
  else   
  if pos('key violation',cBuf) > 0 then  lMsgErr.Caption := FKDUP
  else
  if pos('another user',cBuf) > 0 then  lMsgErr.Caption  := FKLOCK
  else   
  if pos('default value',cBuf) > 0 then  lMsgErr.Caption := FKNULO;
  
  ErrorMsg.Lines.Add(FError.Message);
