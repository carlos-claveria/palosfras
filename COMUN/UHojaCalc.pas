...
// CCD Los campos con Tag -1 no se muestran

// CCD Añado la funcion OOCalc(q : DataSet);
// Que lanza un dataset a una hoja de cálculo
...
USES
  ...
  {$ELSE}
  {$ENDIF}
  SysUtils, ComObj, Classes, Graphics, Controls,db,forms,dialogs;
...
procedure OOCalc(q : TDataSet; nombre : string = 'smhc'; EsExcel : boolean = FALSE);
...
begin
   ...
   if EsExcel then
      x := THojaCalc.Create(thcExcel, true)
   else
      x := THojaCalc.Create(thcOpenOffice, true); 
   ...
   try
    ...
    x.FileName:= 'D:\nuevaxls'; //Needs a file  

    x.ActivateSheetByIndex(1);
    x.ActiveSheetName := nombre;
    ...
    while campos <= q.Fields.Count-1 do begin
    
         if q.Fields[campos].Tag >= 0 then
         begin
            x.CellText[1,i+1] := q.Fields[campos].FieldName;
            x.Bold(l,i+1);
            inc(i);
         end;
    ...
    while not q.Eof do begin
        ...
        while campos <= q.Fields.Count-1 do begin
          ...
          begin
               ...
               if q.Fields[campos].IsNull then x.CellText[l,i+1] := ''
               else
               if q.Fields[campos].IsBlob then x.CellText[l,i+1] := '<no exportable>'
               else
               case q.Fields[campos].DataType of


                      ftSmallint,
                      ftInteger,
                      ftWord      : begin
                                       x.SendNumber(l,i+1,q.Fields[campos].AsInteger);
                                       x.HorizontalAlignment(l,i+1,taRightJustify);
                                       x.NumberFormat(l,i+1,IntToStr(UHojaCalc.nfNUMBER));
                                    end;

                      ftFloat     : begin
                                       x.SendNumber(l,i+1,q.Fields[campos].AsFloat);
                                       x.HorizontalAlignment(l,i+1,taRightJustify);
                                    end;
                      ...
                      ftCurrency  : begin
                                       x.SendNumber(l,i+1,q.Fields[campos].AsFloat);
                                       x.HorizontalAlignment(l,i+1,taRightJustify);
                                       x.NumberFormat(l,i+1,IntToStr(UHojaCalc.nfCURRENCY));
                                    end;
                      ftBCD       : begin
                                       x.SendNumber(l,i+1,q.Fields[campos].AsFloat);
                                       x.HorizontalAlignment(l,i+1,taRightJustify);
                
                                       p := TBCDField(q.Fields[campos]).Precision;
                                       if (p = 2) then begin
                                          x.NumberFormat(l,i+1,IntToStr(UHojaCalc.nfCURRENCY));
                                       end
                                       else
                                       ...
                                       begin
                                         x.NumberFormat(l,i+1,IntToStr(UHojaCalc.nfALL));
                                       end;
                                 

                                    end;
 
                      ftDate      : x.CellText[l,i+1] := FormatDateTime('DD/MM/YYYY',q.Fields[campos].AsDateTime);
                      ftDateTime  : x.CellText[l,i+1] := FormatDateTime('DD/MM/YYYY HH:NN:SS',q.Fields[campos].AsDateTime);
                  
                
               else 
                      try
                        x.CellText[l,i+1] := q.Fields[campos].AsString;
                      except
                        x.CellText[l,i+1] := '<?>';
                      end;
    ...
    if not esExcel then
      while X.StillConnectedToApp do begin
         Application.ProcessMessages;
         Sleep(1000);
      end;
